define("views/AutoCompleteView", ["chaplin", "underscore", "jquery", "es6-promise", "tinyscrollbar"], function(Chaplin, _, $, ES6Promise) {
    ...
    for (var i = 0, refLength = reference.length; i < refLength; i++){
        ...
        for (var j = 0, letLength = letters.length; j < letLength; j++){
            diacriticMap[letters[j]] = reference[i].base;
        }
    }
    function diacriticReplace(s) {
        var result = "";
        for(var c= 0, l= s.length; c<l; ++c) {
            result += diacriticMap[s[c]] || s[c];
        }
        return result;
    }
    return Chaplin.View.extend({
        ...
        "initialize": function(options) {
            ...
            this.formatItem = options.formatItem;
            this.formatNoMatches = options.formatNoMatches;
            this.formatLoading = options.formatLoading;
            this.formatDidYouMean = options.formatDidYouMean;
            this.formatMore = options.formatMore;
        ...
        "search": function(e){
            ...
            if (this.searchString === searchString) {
                ...
                this.selectedItem = resp.selectedItem || this.selectedItem;
                this.results = resp.results;
                this.render();
                return;
            }
            var lowerSearchString = diacriticReplace(searchString.toLowerCase());
            var words = _.filter(lowerSearchString.split(" "), function(word) {
        ...
        "generateUrl": function() {
            ...
            url = url.replace("%(searchString)s", encodeURIComponent(this.searchString));
            if (this.pageable) {
                url = url.replace("%(page)d", this.page);
                url = url.replace("%(pageSize)d", this.pageSize);
            }
            return url;
        },
        "doRequest": function() {
        ...
        "showLoading": function() {
            ...
            var resultsElement = this.getResultsElement();
            var itemElement = $(document.createElement("li"));
            itemElement.addClass("loading-item");
            itemElement.html(this.formatLoading);
            resultsElement.append(itemElement);
        },
        "hideLoading": function() {
            this.$(".loading-item").remove();
            this.$("input").removeClass("loading");
        },
        "renderResults": function(response) {
            var incremental = this.page > 1;
            var results = !incremental ? [] : this.results;
            var query = response.query || "";
            if (query.length) {
                response.results = _.sortBy(response.results, function(result) {
                    return this.getFormattedString(result).toLowerCase().indexOf(query.toLowerCase()) === -1;
                }, this);
            }
            if (response.results.length === 0 && response.suggestions.length > 0) {
                response.results = [{"text": _.result(this, "formatDidYouMean"), "highlight": false, "selectable": false}];
                response.results = response.results.concat(_.map(response.suggestions, function(suggestion) {
                    return {"text": suggestion, "highlight": false};
                }));
            }
            response.results = _.map(response.results, function(result) {
                if (result.id && result.id.indexOf(response.id_prefix) === -1) {
                    result.id = response.id_prefix+result.id;
                }
                return result;
            });
            if (response.prefetch) {
                this.cache[query.toLowerCase()] = _.clone(response);
            }
            this.cacheResponse[query.toLowerCase()] = _.clone(response);
            results = this.prepareResults(results.concat(response.results), query);
            if (!incremental) {
                this.selectedItem = results.selectedItem || 0;
            }
        ...
        "render": function() {
            ...
            this.updateWidth();
            if (incremental) {
                resultsElement.children().last().remove();
            } else {
                resultsElement.children().remove();
            }
            if (results.length === 0 && !incremental) {
                itemElement = $(document.createElement("li"));
                itemElement.html(_.result(this, "formatNoMatches", "No results found :("));
                resultsElement.append(itemElement);
                resultsContainer.data("plugin_tinyscrollbar").update();
            } else {
                var children = resultsElement.children();
                children.removeClass("selected-item");
                var specials = /[.*+?|()\[\]{}\\$^]/g; // .*+?|()[]{}\$^
                var searchStringEscaped = searchString.replace(specials, "\\$&");
                searchStringEscaped = diacriticReplace(searchStringEscaped);
                searchStringEscaped = new RegExp("(" + searchStringEscaped.split(" ").join("|") + ")", "gi");
                for(var c = children.length, l = results.length; c<l; ++c) {
                    var item = results[c];
                    itemElement = $(document.createElement("li"));
                    var itemText;
                    if (item.highlight === false) {
                        ...
                        itemText = item.text.replace(searchStringEscaped, "<b>$1</b>");
                    }
                    itemElement.html(itemText);
                    resultsElement.append(itemElement);
                }
                resultsContainer.data("plugin_tinyscrollbar").update("relative");
                this.renderSelectedItem();
            }
            var doRequest = this.generateUrl() !== this.oldUrl;
            if (doRequest) {
                this.doRequest();
            } else {
                this.lockMore = false;
        ...
        "renderSelectedItem": function() {
            var resultsContainer = this.getResultsElement();
            var scrollbar = resultsContainer.data("plugin_tinyscrollbar");
            var resultsElement = $("ul", resultsContainer);
            ...
            var height = _.reduce(prevAll, function(old, element){ return old + $(element).outerHeight(); }, 0);
            var top = resultsElement.css("top");
            top = parseInt(top.substring(0, top.length-2));
            var movingUp = height < resultsContainer.height()+top;
            var movingDown = height+top > 0;
            if (movingUp && movingDown) { // Probably outbound
                return;
            }
            var aggregate = function(old, element){ return old + $(element).outerHeight(); };
            for (var c = 1; c < this.pageSize; c++) {
                var margin = _.reduce(
                    prevAll.slice(-this.pageSize+c),
                    aggregate,
                    0
                );
                if (height-margin >= scrollbar.contentPosition) {
                    height -= margin;
                    break;
                }
            }
        ...
        "selectItem": function() {
            ...
            if (this.results[this.selectedItem].id) {
                ...
                this.$("input").val(this.results[this.selectedItem].text.replace(/<\/?b>/g, ""));
                this.search({"keyCode": -1});
            }
        },
        "scroll": function() {
            var plugin = this.getResultsElement().data("plugin_tinyscrollbar");
            if(plugin.thumbPosition+plugin.thumbSize === plugin.trackSize) {
                this.loadMore();
            }
        },
        "loadMore": function() {
            if (this.pageable) {
                if (this.lockMore || !this.more) {
                    return false;
                }
                this.lockMore = true;
                var more = $(document.createElement("li"));
                more.html(_.result(this, "formatMore", "Loading more..."));
                $("ul", this.getResultsElement()).append(more);
                this.page++;
                this.doRequest();
            }
            return true;
        ...
        "down": function() {
            if (this.selectedItem >= this.results.length-1-this.preloadBefore && this.loadMore()) {
                return;
            }
